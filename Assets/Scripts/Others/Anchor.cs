...
public class Anchor : MonoBehaviour {
	...
	IEnumerator UpdateAnchorAsync() {
		...
		if (cameraWaitCycles > 0) {
			print(
				$"CameraAnchor found ViewportHandler instance after waiting {cameraWaitCycles} frame(s). You might want to check that ViewportHandler has an earlie execution order.");
		}
		UpdateAnchor();
