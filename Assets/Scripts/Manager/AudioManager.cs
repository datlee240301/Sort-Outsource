...
public class AudioManager : Singleton<AudioManager>
{
    ...
    private static readonly WaitForSeconds _timeWait = new (0.44f);
    ...
    public static void PlayDelaySound(string name)
    {
        if (IsSoundEnable)
        {
            foreach (Sound s in Instance.sounds)
            {
                if (s.Name == name)
                {
                    Instance.StartCoroutine(PlaySoundWithDelay(s));
                }
            }
        }
    }
    
    private static IEnumerator PlaySoundWithDelay(Sound sound)
    {
